...
// @access  Public
exports.login = asyncHandler(async (req: Request, res: Response, next: any) => {
...
const sendTokenInResponse = (user: any, statusCode: number, res: Response) => {
  const token = user.getSignedJwtToken();

  const option = {
    expires: new Date(
      Date.now() + sanitizedConfig.JWT_COOKIE_EXPIRE * 24 * 60 * 60 * 1000
    ),
...
const getResetPasswordToken = function (email: string) {
  // Generate token
